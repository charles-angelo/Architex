    ...
    selectLot(lot) {
        this.activeLot = {
            ...
            address: lot.block && lot.name ?
                ` ${lot.block}, ${lot.name}` : 'Address not available',
            type: lot.type ?? 'N/A',
...
}" x-init="const img = $el.querySelector('.sitemap-image');
if (img.complete) {
...
img.onload = () => {
    ...
    <div x-show="activeTab === 'Sitemap'" x-transition>
        <div class="grid items-start grid-cols-1 gap-10 lg:grid-cols-2">
                ...
                <template x-if="!activeLot">
                    <div class="text-center py-60">
                        ...
                        <p class="max-w-md mx-auto text-gray-600">
                            Click on any highlighted lot from the sitemap to view details such as size, price, and
                            availability.
                        </p>
                    </div>
                </template>
                ...
                <div x-data="{ showFloorPlan: false }">
                    <template x-if="activeLot">
                        <div class="space-y-6 transition-all duration-500 ease-in-out"
                                    ...
                                    <div class="relative w-full overflow-hidden bg-gray-200 aspect-video">
                                        <div class="relative w-full h-full group">
                                            ...
                                            <div class="absolute inset-0 flex items-center justify-between p-4">
                                                <button @click="prev"
                                                    class="p-2 text-white rounded-full bg-black/50 hover:bg-black/70">
                                            ...
                                            <div x-show="!showThumbs"
                                                ...
                                                <template x-for="(image, index) in house_details"
                                                    ...
                                                    <div @click="current = index"
                                                        ...
                                                        <img :src="image"
                                                            class="object-cover w-full h-24 aspect-square">
                                                    </div>
                                                </template>
                                            </div>
                            ...
                            <div>
                                ...
                                <p class="text-gray-700" x-text="activeLot.highlights || 'Information coming soon.'">
                                </p>
                            </div>
                        </div>
                    </template>
                    ...
                    <div class="relative z-10 flex flex-col w-full h-full gap-5 bg-white"
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div x-data="{ selectedLot: null }" @open-reserve-modal.window="selectedLot = $event.detail.lot"
                                class="flex gap-5">
            ...
            <div class="relative overflow-hidden w-full h-[900px]" x-data="{
                hoveredLot: null,
                mapWidth: 800,
                mapHeight: 900,
                zoom: 1,
                ...
                @mouseup.window="endDrag" @touchmove.window="drag" @touchend.window="endDrag"
                @wheel.prevent="wheelZoom($event)" style="user-select: none; touch-action: none;">

                <!-- Zoom Controls -->
                <div class="absolute z-0 flex gap-2 top-2 right-2">
                    <button @click="zoomIn"
                        class="px-3 py-1 text-white bg-green-600 rounded hover:bg-green-700">+</button>
                    <button @click="zoomOut"
                        class="px-3 py-1 text-white bg-red-600 rounded hover:bg-red-700">-</button>
                </div>

                <!-- Map Container -->
                <div class="relative origin-top-left cursor-grab" :class="{ 'cursor-grabbing': dragging }"
                    @mousedown="startDrag($event)" @touchstart="startDrag($event)"
                    :style="`transform: translate(${offsetX}px, ${offsetY}px) scale(${zoom}); transform-origin: 0 0; width: ${mapWidth}px; height: ${mapHeight}px; will-change: transform;`">

                    <img src="{{ asset('img/properties/sitemap3.svg') }}" alt="Sitemap"
                        ...
                        <div
                            ...
                            @if ($isForSaleOrRent)
                                ...
                                <button @click="selectLot(lot)"
                                    class="flex items-center justify-center w-6 h-6 text-xs font-semibold text-white transition transform rounded-full hover:scale-110 hover:shadow-lg focus:outline-none"
                                    :class="{
                                    ...
                                    x-text="lot.id" @mouseenter="hoveredLot = lot.id" @mouseleave="hoveredLot = null"
                                    @touchstart="hoveredLot = lot.id" @touchend="hoveredLot = null">
                                </button>
                            @else
                                <!-- SOLD Indicator -->
                                ...
                                <button @click="selectLot(lot)"
                                    class="absolute transition-all duration-300 -translate-x-1/2 -translate-y-1/2 rounded-full cursor-pointer"
                                    :style="`
                                                                                        width: 32px;
                                                                                        height: 32px;
                                                                                    ...
                                                                                    `"
                                    @mouseenter="hoveredLot = lot.id" @mouseleave="hoveredLot = null"
                                    @touchstart="hoveredLot = lot.id" @touchend="hoveredLot = null">
                                </button>

                                <!-- Tooltip -->
                                <div x-show="hoveredLot === lot.id" x-transition
...
<style>
    /* Hover effect for invisible lots (no Alpine reactivity needed) */
    button.hovered {
        box-shadow: 0 0 20px 8px rgba(130, 180, 80, 0.9);
